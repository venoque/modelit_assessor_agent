...
def display_state(
    ...
    try:
        ...
        task_number = session.state.get("row_index", 0)
        print(f"The task #: {task_number+1}")
        finished = session.state.get("finished", False)
        print(f"If the data set reached the end: ", finished)
...
async def send_message_to_agent(message, agent_url):
